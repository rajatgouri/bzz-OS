...
export default function BeelineFileLogger() {
  const entity = "beelinefilelogger"; // entity api name , don't change it
  ...
  const dataTableColumns = [
    ...
    },
    {
      ...
      filteredValue:filteredValue['File'] || null,
      render: (value) => <div >
    ...
    
    {
      ...
      render: (text, row) => { 
        return {
          children: (
            <div>
               { text.split('T')[0].split('-')[1] + "-" + text.split('T')[0].split('-')[2] + "-" + text.split('T')[0].split('-')[0]  + " " + text.split('T')[1].slice(0,8  ) }
            </div>
            ),
        }
      }
  ...
  const DATATABLE_TITLE = "Beeline File Logger";
